...
### ⚡ Daily To Do
Notion day pages. Swipe right = defer to tomorrow; swipe left = delete (one-off only). Recurring undeletable. Category labels display below task name. Checked tasks sink to bottom (not draggable). When shopping items exist, a "Shopping List" link row appears at the bottom — tap to open. All task text displayed uppercase.

**Drag to reorder (`@dnd-kit`).** Grab the whole tile — desktop: click-drag; mobile: brief press-and-hold then drag (the hold is what separates a reorder from a tap/swipe). Drop on another task to reorder, or on a **Week Ahead day row** to move it to that day. One unified `DndContext` spans both the Daily To Do and Week Ahead cards, so reorder and move-to-day are the same gesture (custom `dndCollision`: a day row under the pointer wins move-to-day, otherwise collisions are constrained to the dragged task's own group). Unchecked non-pinned tasks reorder in a **per-day** manual order; **pinned (📌) tasks reorder among themselves** in their own bottom group under a **global** order that holds across every day. Order persists via `/api/task-order`, with the same 15s pending-write guard as checked-state so a stale Notion read can't undo a fresh drag. Tasks in a supplier order-link (`SUPPLIER_LINKS` in `page.tsx`) open a prefilled action — e.g. Planetware → `tel:` call, Candied → `mailto:` with a prefilled bakery order.
...
| `/api/task-order` | GET/POST | Read/write the Daily To Do manual drag-to-reorder map (JSON in a `yaml`-tagged code block on the OS page, kept separate from the `json` checked-state block). Per-date keys hold the normal-task order; a global `_pinned` key holds the pinned-group order |
